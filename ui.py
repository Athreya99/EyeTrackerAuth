...
def success():
    ...
    lb1=tk.Label(root, text="AUTHENTICATION SUCCESSFUL!",width=50,font=("Century Gothic",40,"bold","italic"),foreground="white",bg="black")
    lb1.place(x=-50,y=80)
    button2=Button(root,text="Exit",borderwidth=10, relief="raised",command=root.destroy,width=20,font=("Century Gothic",24,"bold","italic"),foreground="white",bg="black")
    button2.place(x=550,y=700)
    tk.mainloop()
def failure():
    pop1=Tk()
    result=tkinter.messagebox.askquestion('Authentication Failed','Do you want to retry?')
    if result=='no':
        pop1.destroy()
        quit()
    pop1.mainloop()
def run():
    ...
    board = np.zeros((100,450), np.uint8)
    board[:] = 255
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor("D:\downloads upto january\shape_predictor_68_face_landmarks.dat")
    ...
    def draw_menu():
        ...
        cv2.line(keyboard, (int(cols/2) - int(th_lines/2), 0),(int(cols/2) - int(th_lines/2), rows),
                 (51, 51, 51), th_lines)
        cv2.putText(keyboard, "(0-4)", (80, 300), font, 6, (255, 255, 255), 5)
        cv2.putText(keyboard, "(5-9)", (80 + int(cols/2), 300), font, 6, (255, 255, 255), 5)
    ...
    to="+1 999999999",      #phone number to receive twilio message
    from_="+1999999999" ,  
    ...
    while True:
        ...
        cv2.putText(board,('*'*len(text)), (50,100), font, 9, 0, 3)
        ...
        cv2.moveWindow('keyboard',8000,800)
...
window = tk.Tk()
window.title("Eye Tracker Password")
...
ima = PhotoImage(file = 'front.png')
label = ttk.Label(window, image = ima)
label.place(x=0,y=-100)

lb1=tk.Label(window, text="Eye Tracking",width=50,font=("Century Gothic",40,"bold","italic"),foreground="white",bg="black")
lb1.place(x=-50,y=80)

button1=Button(window,text="Launch",command=run,borderwidth=10, relief="raised",width=20,font=("Century Gothic",24,"bold","italic"),foreground="white",bg="black")
button1.place(x=100,y=300)

button2=Button(window,text="Exit",borderwidth=10, relief="raised",command=window.destroy,width=20,font=("Century Gothic",24,"bold","italic"),foreground="white",bg="black")
